...
import biApplicantSubmitRoutes from "./routes/biApplicantSubmitRoutes";
...
async function bootstrapInner() {
  ...
  try {
    ...
    // BI_SERVER_BLOCK_v472_WORKER_SWITCH_v1 - no scheduled jobs on the staging slot
    if (!workersEnabled()) logger.warn("BI scheduled jobs disabled by BI_WORKERS_ENABLED");
...
app.use("/api/v1", biCors, biApplicantSubmitRoutes); // BI_CLIENT_SUBMIT_v25
