...
router.post("/api/v1/webhooks/pgi", express.raw({ type: "application/json" }), async (req, res) => {
  ...
  if (evt.event === "application.quoted") {
    ...
    await pool.query(`UPDATE bi_applications SET status='approved', quote_id=$1, underwriter_ref=$2, annual_premium=$3, quote_valid_until=$4, updated_at=NOW() WHERE pgi_application_id=$5`, [evt.quote_id, evt.underwriter_ref, evt.annual_premium, evt.valid_until, evt.application_id]);
    if (prev.rows[0] && prev.rows[0].status !== "approved") await onApplicationApproved(prev.rows[0].id);
