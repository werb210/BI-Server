...
export type CarrierHealth = {
  status: "healthy" | "degraded" | "pending" | "idle" | "unknown";
...
export async function getCarrierHealth(): Promise<CarrierHealth> {
  ...
  const submissions = Number(subs.rows[0]?.n || "0");
  const received = Number(recv.rows[0]?.n || "0");
  const errors = Number(errs.rows[0]?.n || "0");

  let status: CarrierHealth["status"];
  let reason: string;
  if (errors > 0) {
    status = "degraded";
    reason = `${errors} carrier_submission_failed / pgi_webhook_failed events in last 24h`;
  } else if (submissions === 0) {
    status = "idle";
    reason = "No submissions in last 24h (carrier not exercised)";
  } else if (received === 0) {
    status = "pending";
    reason = `${submissions} submission(s) sent, awaiting carrier ack — normal during low volume`;
  } else {
    status = "healthy";
    reason = `${received} of ${submissions} submission(s) acknowledged, 0 errors`;
  }
  ...
  return {
    ...
    errors_24h: errors,
    last_received_at: lastRecv.rows[0]?.t || null,
    last_error_at: lastErr.rows[0]?.t || null,
    reason,
