...
export interface BrandedEmailTemplate {
  ...
  // BI_EMAIL_SECOND_COLUMN_CTA_v1 - mirrors BF_EMAIL_SECOND_COLUMN_CTA_v1.
...
// BI_EMAIL_SECOND_COLUMN_CTA_v1 - a button sized for a 264px column rather than
// the 544px frame, so it does not overflow once the columns stack on a phone.
...
function column(headline: string, body: string, imageUrl: string, imageLink: string, ctaLabel = "", ctaUrl = ""): string {
  ...
  return `${heading}${linkedImage}${copy}${columnCta(ctaLabel, ctaUrl)}`;
}
...
<td class="email-column" width="264" valign="top" style="width:264px;">${column(template.headline || "", template.body || "", template.heroUrl || "", template.heroLink || "", template.ctaLabel || "", template.ctaUrl || "")}</td>
