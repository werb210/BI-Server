...
// BI_SERVER_BLOCK_73_APOLLO_LIST_FORK_v1 - Companies-list import path.
// Apollo's /mixed_companies/search is the Companies-side counterpart of
// /mixed_people/search and accepts label_ids the same way. Used by the
// admin list-import handler to load org records when a label points at
// a Companies list rather than a People list.
...
export async function searchCompaniesByLabel(args: { label_ids: string[]; page?: number; per_page?: number }): Promise<{ organizations: ApolloOrganization[]; pagination: { page: number; per_page: number; total_entries: number; total_pages: number } }> {
  const body: Record<string, unknown> = {
    page: args.page ?? 1,
    per_page: Math.min(args.per_page ?? 100, 100),
    label_ids: args.label_ids,
  };
  const res = await apolloRequest<any>("/mixed_companies/api_search" /* v345 */, { method: "POST", body });
